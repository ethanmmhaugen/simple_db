...
Pager* pager_open(const char* filename) {
    ...
    long file_length = _lseek(fd, 0, SEEK_END);
    ...
    if (file_length % PAGE_SIZE != 0) {
        printf("DB file is not a whole number of pages. Corrupted.\n");
        exit(EXIT_FAILURE);
    }
...
void* get_page(Pager* pager, uint32_t page_num) {
    ...
    if(pager->pages[page_num] == NULL) {
        ...
        if(page_num <= num_pages) {
            ...
            ptrdiff_t bytes_read = read(pager->file_descriptor, page, PAGE_SIZE);
            if(bytes_read == -1) {
...
void pager_flush(Pager* pager, uint32_t page_num) {
    ...
    ptrdiff_t  bytes_written = write(pager->file_descriptor, pager->pages[page_num], PAGE_SIZE);
